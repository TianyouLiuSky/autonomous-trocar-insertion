...
The diagnostic also fits the ChArUco board-frame-origin arc generated by the
orientation
sweep. This is a no-contact alternative to a physical pivot test:

- Fixed orientation-derived rotation gives a much lower arc residual:
  investigate the coordinate basis used for FrameEE XYZ.
- Fixed spatial-derived rotation gives a much lower arc residual:
  investigate the orientation or board-rotation pathway.
- Similar fixed-rotation residuals, large competitive optimizer spread, or
  physically implausible fitted offsets: the arc is inconclusive;
