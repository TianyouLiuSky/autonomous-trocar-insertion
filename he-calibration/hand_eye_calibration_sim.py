...
class RealSenseCamera:
    ...
    def stop(self):
        ...
        #TODO edited here
        # Fake intrinsics (standard 720p camera)
        self.K = np.array([[900, 0, 640], [0, 900, 360], [0, 0, 1]])
        self.dist = np.zeros(5)
        print("MOCK Camera initialized (No Hardware)")

    def get_frame(self):
        # Return a blank gray image so the GUI display works
        return np.zeros((720, 1280, 3), dtype=np.uint8) + 128
...
class CalibrationGUI(QtWidgets.QWidget):
    def __init__(self):
        ...
        #TODO edited here
        # GROUND TRUTH (The "Secret" answer the calibration should find)
        # Position: 40cm Right (-Y), 20cm toward user (+X), 30cm Up (+Z)
        self.TRUE_T_cam2base = np.eye(4)
        
        # Translation (in meters)
        # X=0.2 (toward user), Y=-0.4 (right), Z=0.3 (up)
        self.TRUE_T_cam2base[:3, 3] = [0.2, -0.4, 0.3] 

        # Rotation: Looking "Forward"
        # If the camera's Z-axis (pointing out of lens) is parallel to the 
        # Robot's X-axis (looking "past" the robot rather than at it):
        # We use Identity if the axes align, or a rotation if they are offset.
        # Assuming the camera is looking straight ahead (parallel to Robot X):
        self.TRUE_T_cam2base[:3, :3] = np.eye(3)
    ...
    def update(self):
        ...
        # TODO edited here.
        # SIMULATED DETECTION BRIDGE
        robot_pose = self.robot.get_pose()
        if robot_pose is not None:
            # 1. Convert robot pose to Matrix (T_target2base)
            R_robot = Rotation.from_quat(robot_pose['q']).as_matrix()
            t_robot = robot_pose['t'].reshape(3, 1)
            T_target2base = np.eye(4)
            T_target2base[:3, :3] = R_robot
            T_target2base[:3, 3] = t_robot.flatten()

            # 2. Calculate what the camera sees (T_target2cam = T_base2cam * T_target2base)
            T_base2cam = np.linalg.inv(self.TRUE_T_cam2base)
            T_target2cam = T_base2cam @ T_target2base

            # 3. Convert back to vectors for the engine
            rvec, _ = cv2.Rodrigues(T_target2cam[:3, :3])
            tvec = T_target2cam[:3, 3]
            corners = None 
        else:
